...
Not modelled
------------
Solar and geomagnetic activity, the diurnal bulge, winds, and any seasonal or latitudinal
variation. The table is a single static mean profile. Real thermospheric density at 400 km varies by
more than an order of magnitude over a solar cycle, which is larger than the difference between the
two laws offered here. `pymsis` (NRLMSISE-00) is the right dependency for that and is deliberately
out of scope; `CLAUDE.md` forbids reimplementing it.
...
__all__ = [
    ...
    "layered_density", "exponential_density",
]
...
# float array; the kernel tests `>= 0.5`, so any value rounds to the nearer law rather than silently
# selecting neither.
...
_N_BANDS: Final[int] = int(BASE_ALTITUDE_KM.size)
