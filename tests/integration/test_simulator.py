...
def test_simulation_initialization(db_session):
    ...
    earth = CelestialBodyORM(name="Earth", mu=398600.4418, radius=6371.0, parent_id=None,
                            p=149597870.7*(1 - 0.0167**2), e=0.0167, i=m.radians(0.00005), raan=0.0, arg_pe=0.0, theta=0.0)
    db_session.add(earth)
    db_session.flush()
    moon = CelestialBodyORM(name="Moon", mu=4902.800066, radius=1737.1, parent_id=earth.id, 
    ...
    sim = Simulation(body_names=["Earth", "Moon"], system_names=[], session=db_session)
    ...
    assert sim.parent_indices[sim.name_to_index["Earth"]] == sim.name_to_index["Earth"] # Earth has no parent so should self reference.
