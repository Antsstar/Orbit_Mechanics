...
| `zonal.py` | Force model `"zonal"` (`ZONAL_MODEL`): the J3..J6 zonal perturbation of the `parent_indices` parent, **additive to `"j2"`, never a replacement** - a J2..J6 config enables both, `"j2"` keeps its fused twin and `SECULAR_J2` keeps reading `force_model_params["j2"]`. Coefficients `(r_eq, j3, j4, j5, j6)` (unnormalised `J_n`) on the perturbed body's row; spin axis = frame +z. Kernel: Legendre recursions, `a_n = (mu/r^2) J_n (R/r)^n [P'_{n+1}(s) r_hat - P'_n(s) z_hat]`; a zero `J_n` adds exactly nothing. EGM96 `EARTH_J3`..`EARTH_J6` (`EARTH_ZONALS` is the keyword set) from `EGM96_CBAR_N0` via `J_n = -sqrt(2n+1) Cbar_n0`, **from memory, unverified** - the same table's `Cbar_20` reproduces `EARTH_J2` within its printed digits (`EARTH_J2` is the truncation, 5.5e-14 low). Refuses barycentre parents, non-finite coefficients and `r_eq <= 0` on any row whose effective `J_n` is non-zero. **Fused into the compiled Cowell twin** (`kernels._cowell_accel`, per-body `has_zonal`, the same recursion in the same order): Cowell + pm + j2 + zonal stays compiled at **1.17x** the fused `j2` tier (0.0477 s against 0.0406 s at 15 s over 24 h; the NumPy fallback was ~120x), held to NumPy at 6.3e-13 relative over 1440 steps. A zonal body carrying a model outside the fused set (`srp`, `third_body`, `thrust`) still falls back; drag is fused too. Validated in `tests/validation/test_zonal.py`: against `reference.zonal_field` (<= 8.9e-14 of scale), a finite-differenced potential, bitwise parity, fourth-order convergence on zonal truth, and differentially against J2 twins - J3 `de/dt` +0.42 %, J4 `dRAAN/dt` +0.97 % against a 2 % budget. Headline (`benchmarks/zonal_sweep.py`, 12 sats, 550 km / 53 deg, 24 h, J2..J6 truth): omitting J3..J6 costs **1.26 km** median (RK4 at 15 s: 2.7e-3 km; at 60 s the 1.88 km truncation is larger) but only **0.07 s mean / 0.59 s max** rise shift, no pass lost - under the 1 s pad |
| `tesseral.py` | Force model `"tesseral"` (`TESSERAL_MODEL`): orders **m >= 1 of degrees 2..4** (nine C/S pairs) of the `parent_indices` parent, **additive to `"j2"` and `"zonal"`** (a 4x4 field enables all three). Coefficients `(r_eq, omega, theta0, c21, s21, ..., c44, s44)`, **unnormalised**; `EARTH_TESSERALS` / `EARTH_J22` from `EGM96_CS_BAR` via `unnormalise` (`N_nm = sqrt((2-delta_m0)(2n+1)(n-m)!/(n+m)!)`), **from memory, unverified** - normalisation checked against a remembered JGM-3 table to <= 1.5e-3. **The first kernel that reads `t`**: `theta = theta0 + omega * t`, **absolute sim time** (`geometry.py`'s convention at `epoch_s = 0`); body-fixed = `Rz(-theta)` of inertial. Kernel: Cunningham V/W recursion (Montenbruck & Gill 3.2.4-3.2.5, numbers unverified). Refuses barycentre parents, non-finite values, and `r_eq <= 0` **or `omega == 0`** on a row with any non-zero C/S (an unwritten omega silently freezes the field). Zero coefficients add exactly nothing. **Not fused**: a Cowell body with it sends the whole set down NumPy (~320x the fused tier on the LEO sweep); wants a `kernel-twin` (needs `t` in `_cowell_accel`). Validated in `tests/validation/test_tesseral.py`: truth field <= 1.5e-12 of scale, FD potential, co-rotating invariance, **fourth order (16.6) vs frozen stage time first order (1.995)** - the only tests that catch an RK4 stage-time bug - and GEO: J22 drift `+18 n^2 (R/a)^2 J22 sin 2(lambda - lambda22)` (plus sign, `lambda22 = atan2(S22,C22)/2` = -14.93 deg the long axis) within 8.2e-5 K of theory. Headline (`benchmarks/tesseral_sweep.py`): 4x4 stable slots **74.94 E / 105.09 W** (published 75.1 / 105.3, a comparison), east-west station keeping **1.764 m/s/yr** worst slot under J22, **2.066** under 4x4 (at 117.4 E); at LEO (550 km / 53 deg, 24 h) omitting tesserals costs **3.8 km median / 9.0 max** (3x J3..J6) and 0.29 s mean / **1.15 s max** rise shift, no pass lost |
| `drag.py` | Force model `"drag"`: `-(1/2) rho B |v_rel| v_rel`, with `v_rel = v - w x r` relative to `parent_indices` and the atmosphere co-rotating about the frame's +z (the same assumption as `j2`). Per-body coefficients are `(ballistic_coeff m^2/kg, rho0 kg/m^3, h0 km, scale_height km, r_ref km, omega rad/s, density_model, f107, f107a, ap)` - the last three read **only** on `DENSITY_MODEL_MSIS` rows, so a legacy 7-column array still works, and the one unit conversion (`rho*B` from 1/m to 1/km, x1e3) sits in the kernel. `EARTH_OMEGA` is provided. `scale_height <= 0` means a silent zero contribution **under the single-band law only**. Refuses barycentre parents via `validate_bodies`; `validate_coefficients` refuses an unknown `density_model`, MSIS without **all three** indices in the same call (so `pymsis` never downloads them), solar indices on a non-MSIS row, and out-of-range indices - then evaluates the MSIS profile, before any bit is set. **Fused into the compiled Cowell twin** (`kernels._cowell_accel`, per-body `has_drag`, every density law, the same arithmetic in the same order, each RK4 stage fed its own stage velocity `pv + v_k` exactly as `RK4Integrator` writes it): the layered table and each MSIS triple's memoised profile are stacked at plan time by `drag.density_tables` (row 0 the table, one row per distinct triple, `_cowell_drag_table_of` per body) and searched by a scalar binary search reproducing `np.searchsorted(side="right")` and its clipped terminal bands; the selector and every coefficient are read live, and an MSIS row whose live indices no longer match its planned profile makes the kernel return that slot having written nothing, so `step()` re-plans (an unevaluated triple then falls back, and the reference raises `LookupError` as before). Held to `drag_kernel` at **1.7e-14** of `|a_drag|` field-level (bound 1e-12; band edges exactly and an ulp either side, both extrapolated regions, heliocentric parent) and the state to 1.6e-13 by norm over 500 steps. Cost: drag adds ~0.16 us per body-step on top of the fused `j2` tier's 0.076 (all three laws within 5 % of each other: `exp` dominates, the band search does not); the station-keeping figure's 10-day run went **41.3 s -> 9.7 s** and `benchmarks/msis_sweep.py` **317 s -> 19 s**, every printed number unchanged. The Cowell step composes `_gravity_accel`, `_drag_term`, `_zonal_term` itself behind their flags - passing the tables through one per-stage call tripled the pm/j2 tiers' cost. Validated against orbit-averaged decay, energy balance and the co-rotation factor (`tests/validation/test_drag.py`) |
| `atmosphere.py` | The density law `"drag"` uses, selected per body by its `density_model` coefficient - **not** a second force model and **not** a mask bit, so it is one more key in a `sweep.ForceModelSpec`'s `coefficients`. `DENSITY_MODEL_EXPONENTIAL` = 0.0 is the single band and is what an **unwritten (all-zero) row already selects**, so every pre-existing configuration is unchanged; `DENSITY_MODEL_LAYERED` = 1.0 is Vallado 4e Table 8-4, 28 piecewise-exponential bands from 0 to 1000+ km, evaluated by one vectorised `np.searchsorted`. Under the layered law `rho0`, `h0` and `scale_height` are **ignored**. The table is **transcribed from memory and unverified against the text**; its internal check is continuity, which 27 of 28 boundaries pass inside 1e-4 - the 0-25 km boundary closes to 1.4e-3 against a 7.7e-4 rounding bound and is asserted as a **named anomaly**, so treat `H = 7.249 km` as the table's least trustworthy entry. Extrapolates the terminal bands at both ends; a masked row returns exactly 0.0 without evaluating `exp`. Compiled twin: in `drag`'s fused Cowell term (see its row), all three laws. The headline comparison: over 3 days from 355 km at `B = 0.4 m^2/kg` the table predicts **27.1 km more decay** than a single band matched at 355 km with `H = 60 km`, 30 % more, each within 1.9e-4 of its own orbit-averaged mean ODE (`tests/validation/test_atmosphere.py`). **`DENSITY_MODEL_MSIS` = 2.0 is NRLMSIS 2.0 via `msis_bridge.py`** (`[msis]` extra, `pymsis` imported lazily, `version=2.0` pinned): `pymsis` is **never called in a step** - `drag`'s `validate_coefficients` evaluates, once per `(f107, f107a, ap)`, the global mean over latitude (8 Gauss-Legendre), local time (8) and day of year (24) at 601 nodes 0-1000 km (1.3 s), **memoised as a pure function of the triple**; the kernel evaluates it log-linearly through the shared `piecewise_exponential_density` (+29 us/call) and **raises `LookupError`** on an unevaluated triple rather than evaluating lazily. Presets `SOLAR_ACTIVITY_LOW/_MODERATE/_HIGH` (ECSS 65/65/0, 140/140/15, 250/250/45, from memory), `msis_coefficients(...)` for a `ForceModelSpec`. **The average discards** the diurnal bulge (2.30x at 400 km), the season (1.63x) and index time variation - a model choice. Legacy rows **bit-identical** (frozen pre-MSIS kernel; 2000-step run). Table vs moderate MSIS, 150-1000 km: **0.79 (800 km) to 1.28 (160 km)**, a plausibility check, not verification. **Headline** (`benchmarks/msis_sweep.py`, 6 days, station-keeping scenario, baseline MSIS moderate 3.047 m/s/day): solar low **-71.9 %**, high **+134.4 %**, table +12.8 %, single band -3.1 %, each within 6e-4 of its pre-run prediction - **solar activity is 5-10x the atmosphere-model choice**, and the static laws straddle moderate MSIS. Validated in `tests/validation/test_msis.py` / `_wiring.py` / `_delta_v.py` |
...
| `integrators.py` | `Integrator` protocol and `RK4Integrator(max_capacity)`: `step(provider, t, state, dt, indices, primaries)` advances `state[indices]` relative to `state[primaries]`, copying each stage's accelerations (the provider returns a shared buffer). Named stage buffers are allocated once in `__init__`, but fancy indexing still allocates temporaries every stage, so it is not allocation-free. The frozen-primary formulation is exact only for forces that depend on position relative to the parent, which is every registered model except `third_body` (first order, see its row). Compiled twin `kernels.cowell_rk4_step`, fused with `point_mass_gravity`, `j2`, `drag` and `zonal`; `step()` uses it when `use_compiled_kernel` is set **and** `Simulation._cowell_fused_ok` (rebuilt by `_refresh_cowell_plan`: every Cowell body's mask is a subset of those four models, no Cowell body parents another, and every MSIS drag triple has a memoised profile), otherwise this NumPy path runs |
...
| `isl.py` | **Inter-satellite link visibility** - `access.py` for satellite *pairs*, and the ISL half of the downstream contact dataset. A pair links while the **segment** between them clears `R + h_graze_km`: `geometry.segment_clearance(r1, r2, body_radius_km=, h_graze_km=)` is `line_of_sight`'s clamped test as a *continuous signed distance* (km, positive = visible), so edges are inverse-interpolated like elevation. `IslSpec(central_body=, body_radius_km=, h_graze_km=, max_range_km=None, sample_dt_s=60, bodies=None)` - **`h_graze_km` is required, never defaulted** (commonly ~100 km); `max_range_km` windows on `min(clearance, max_range - range)`. Unordered pairs `a < b` in the caller's body order, `pair_indices(n)` = `triu_indices(n, 1)`, vectorised over `(times, pairs)` in pair blocks (`_BLOCK_SAMPLES`). `isl_windows` / `isl_contacts` / `isl_contacts_from_simulation` -> `IslWindow(body_a, body_b, rise_s, set_s, duration_s, min_range_km, peak_time_s, clip flags)` / `IslContact(window, rise, peak, set)` reusing **`access.ContactSample`** (range rate **positive = opening**; peak = sampled closest approach). A thin sibling type, not `AccessWindow`: an ISL has no station and no elevation. **The metric is `access.py`'s**: `compare_isl_windows` adapts to `AccessWindow` (`station_index <- a`, `body_index <- b`) and calls `access.compare_windows`, so `SweepResult.isl` **is** an `AccessMetrics`. `run_sweep(..., isl=spec)` / `isl_metrics_for`: same divisibility rule, one dense truth per sweep (**the access truth itself** on the same grid), `ErrorStats` and `access` bit-identical; `score_external` scores `ExternalTier`s (SGP4) too. **The clearance is concave at its crossing**, so ISL rises read *late*, sets *early* (opposite to elevation): `-(f''/2f') a b`, 0.077 s max at `h=60 s` on `scenarios.coplanar_satellites` 550/1200 km, halving ratio 4.00. **An ISL cannot see a common rotation of the constellation** (Kepler's missing nodal regression). Headline (`benchmarks/isl_sweep.py`, 12 sats 550 km/53 deg, **3 planes of 4**, 24 h, J2..J6 truth, 366 windows on 48 cross-plane pairs): mean/max \|rise shift\| Kepler 20.6/41.6 s (ground 55.7 s), secular-J2 mean-seeded **0.32/0.77 s** (ground 2.07 s), Cowell+j2 60 s 0.125/0.347 s (ground 0.107), Cowell+j2+zonal 60 s 0.082/0.226 s (ground 0.082) - **no ISL window lost or gained by any tier**; the analytic tiers do *better* on links than on ground passes, the Cowell tiers the same. Not a registry entry. Validated in `tests/validation/test_isl.py` **Edge bias of the exported dataset, cross-plane pairs** (review, 3x4 at 550 km, own 1 s oracle): every rise late / set early by **0.45 s mean, 0.65 s max at 60 s**, 0.16 s at 30 s, 0.04 s at 15 s (4.0x per halving) - 8x the coplanar 0.077 s, since cross-plane pairs close faster. Inside `run_sweep(isl=)` it is common-mode and cancels; in the dataset a downstream consumer exports it does not, so sample at 15 s for sub-0.05 s edges |
...
`j2`, `drag` (all three density laws) and `zonal` (per-body flags), because numba cannot dispatch over the Python kernel list
...
| Cowell step (RK4 + `point_mass_gravity` + `j2` + `drag` + `zonal`) | `integrators.RK4Integrator` over `forces.compose_accelerations` | `kernels.cowell_rk4_step` (fused; other masks fall back to the reference) |
