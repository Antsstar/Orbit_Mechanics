...
| `integrators.py` | `Integrator` protocol and `RK4Integrator(max_capacity)`: `step(provider, t, state, dt, indices, primaries)` advances `state[indices]` relative to `state[primaries]`, copying each stage's accelerations (the provider returns a shared buffer). Named stage buffers are allocated once in `__init__`, but fancy indexing still allocates temporaries every stage, so it is not allocation-free. The frozen-primary formulation is exact only for forces that depend on position relative to the parent, which is every registered model except `third_body` (first order, see its row). Compiled twin `kernels.cowell_rk4_step`, fused with `point_mass_gravity`, `j2` and `zonal`; `step()` uses it when `use_compiled_kernel` is set **and** `Simulation._cowell_fused_ok` (rebuilt by `_refresh_cowell_plan`: every Cowell body's mask is a subset of those three models and no Cowell body parents another), otherwise this NumPy path runs |
...
`j2` and `zonal` (per-body flags), because numba cannot dispatch over the Python kernel list
...
| Cowell step (RK4 + `point_mass_gravity` + `j2` + `zonal`) | `integrators.RK4Integrator` over `forces.compose_accelerations` | `kernels.cowell_rk4_step` (fused; other masks fall back to the reference) |
