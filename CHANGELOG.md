...
- **Dynamic Orbital Rehydration:** `_rehydrate_coes` method now dynamically recalculates exact two-body $\mu$ values ($G(m_1 + m_2)$ for Barycentric motion, $G(m_{parent})$ for Point-Mass orphans).
